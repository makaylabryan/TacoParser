...
namespace LoggingKata
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (lines.Length == 0) 
            {
                logger.LogError("Error: No input data.");
                return;
            }
            if (lines.Length == 1) 
            {
                logger.LogWarning("Warning: There is only one line of data.");
            }
            ...
            // Grab an IEnumerable of locations using the
            // Select command: var locations = lines.Select(parser.Parse);
